...
def on_mqtt_message_receive_callback__heater_(th_C3_B4ng_tin):
  global serial, RT, RH, motion, SM, dis
  if th_C3_B4ng_tin == '1':
...
def _C4_90_C4_82NG_K_C3_9D_SERVER():
  global th_C3_B4ng_tin, serial, RT, RH, motion, SM, dis, aiot_lcd1602, aiot_dht20, tiny_rgb, aiot_ultrasonic
...
def on_event_timer_callback_U_h_W_p_G():
  global th_C3_B4ng_tin, serial, RT, RH, motion, SM, dis
  ...
  if serial == 1:
    print((''.join([str(x) for x in ['#', RT, ':', RH, ':', SM, ':', motion, ':', dis, '!']])), end =' ')
  else:
    mqtt.publish('SENSOR', (''.join([str(x2) for x2 in ['#', RT, ':', RH, ':', SM, ':', motion, ':', dis, '!']])))
